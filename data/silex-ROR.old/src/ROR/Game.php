 ...
 * $action (string) : the current forum action, can have any value defined in $VALID_ACTIONS
 * $tick (int) greater than 0 increasing by 1 everytime a change is committed
 ...
 * --- FORUM PHASE ---
 * $currentPlayer (Party) : Party currently playing for FORUM (Initiative) phase
 * $initiativeParty (Party) :  The Party with the current initiative. null if bidding is underway
 ...
 * $firstParty (Party) : The party with the HRAO
 * --- THE DECKS ---
 * $earlyRepublic
 * $middleRepublic
 * $lateRepublic
 ...
 * $event Array (int) : array of events numbers
...
class Game 
{
    ...
    public static $VALID_ACTIONS = array('RollEvent','Persuasion','Knights','ChangeLeaderSponsorGames');
    ...
    private $id ;
    public $name ;
    public $turn , $phase , $subPhase , $action , $tick;
    public $scenario , $unrest , $treasury , $nbPlayers ;
    public $currentPlayer , $initiativeParty ,$persuasionTarget ,$persuasionWith ;
    ...
    public $landBill ,$event , $legion , $fleet ;
     ...
     */
    public function create($name , $scenario , $partyNames) {
        $messages = Array () ;
        ...
        foreach ($partyNames as $key=>$value) {
            ...
            $this->party[$key]->create($value , $key);
        }
        /*
        ...
        array_push($messages , 'Setup phase - The First Punic war is now an inactive war' ) ;
        ...
        array_push($messages , 'Setup phase - Rome starts with 4 Legions' ) ;
        /* 
        ...
        foreach ($this->party as $key=>$value) {
            for ($j=0 ; $j<3 ; $j++)
            {
                $value->senators->putOnTop($tempDeck->drawTopCard());
                array_push($messages , 'Setup phase - Party '.$value->name.' (player [%USER_ID'.$key.'%]) receives Senator '.$value->senators->cards[0]->name ) ;
            }
        }
        ...
        foreach ($this->party as $key=>$party) {
            ...
            array_push($messages , 'Setup phase - Party '.$party->name.' (player [%USER_ID'.$key.'%]) receives 3 cards.') ;
        }
        /*
        ...
        array_push($messages , 'Setup phase - '.$temporaryRomeConsul->name.' becomes temporary Rome consul.');
        return $messages ;
    }
    ...
    public function setup_setPartyLeader( $user_id , $senatorID ) {
        ...
        if ($this->party[$user_id]->leader !== NULL) {
            return Array('ERROR - The leader is already set');
        }
        foreach ($this->party[$user_id]->senators->cards as $senator) {
            if ( ($senator->senatorID == $senatorID) && is_null($this->party[$user_id]->leader) ) {
                ...
                return Array($senator->name.' is the new leader of party '.$this->party[$user_id]->name);
            }
        }
        return Array('Error. Party leader not set.');
    }
    ...
    public function setup_Finished($user_id) {
        $this->party[$user_id]->phase_done = TRUE ;
        return ( $this->whoseTurn() === FALSE ) ? TRUE : FALSE  ;
    }
    ...
    public function mortality() {
        $messages = Array() ;
        array_push($messages , 'Setup phase is finished. Starting Mortality phase.');
        $this->phase = 'Mortality';
        array_push($messages , 'MORTALITY PHASE');
        // Activate imminent wars
        if (count($this->imminentWars->cards)==0) {
           array_push($messages , 'There is no imminent conflict to activate.'); 
        } else {
            ...
            while ( count($this->imminentWars->cards) > 0 ) {
                ...
                array_push($messages , 'Imminent conflict '.$conflict->name.' has been activated.');
        ...
        foreach ($chits as $chit) {
            if ($chit!='NONE' && $chit!='DRAW 2') {
                array_push($messages , 'Chit drawn : '.$chit.'. '.$this->killSenator((string)$chit));
            } else {
                array_push($messages , 'Chit drawn : '.$chit);
            }
        }
        array_push($messages , 'Mortality phase is finished. Starting revenue phase.');
        $this->phase = 'Revenue';
        array_push($messages , 'REVENUE PHASE');
        $this->revenue_init();
        return $messages ;
    }
    ...
    public function killSenator($senatorID) {
        ...
        if (count($deadSenators)==0) {
            return 'This senator is not in Play, nobody dies.' ;
        ...
        if ($party === FALSE) {
            return 'ERROR retrieving the party of the dead Senator';
        }
        if ($deadSenator->type == 'Stateman') {
            ...
            $message.=$deadStateman->name.' ('.$party->name.' - [%USER_ID'.$party->user_id.'%]) dies. The card is discarded. ' ;
        } else {
            // Death of a normal Senator
            $deadSenator->resetSenator() ;
            if ($party->leader->senatorID == $senatorID) {
                $message.=$deadSenator->name.' ('.$party->name.' - [%USER_ID'.$party->user_id.'%]) dies. This senator was party leader, the family stays in the party. ' ;
            } else {
                $deadSenator = $party->senators->drawCardWithValue('senatorID',$senatorID) ;
                $this->curia->putOnTop($deadSenator);
                $message.=$deadSenator->name.' ('.$party->name.' - [%USER_ID'.$party->user_id.'%]) dies. The family returns to the curia. ' ;
            }
        }
        ...
        while (count($deadSenator->controls->cards)>0) {
            ...
            if ($card->type=='Concession') {
                ...
                return 'ERROR - a card controlled by the dead Senator was neither a Family nor a Concession.';
            }
        }
        return $message ;
    }
    ...
    public function revenue_ProvincialSpoils ($user_id , $request ) {
        if ( ($this->phase=='Revenue') && ($this->subPhase=='Base') && ($this->party[$user_id]->phase_done==FALSE) ) {
            ...
            array_push ($messages , $this->party[$user_id]->name.' ([%USER_ID'.$user_id.'%]) gains '.($base['leader']!=NULL ? 3 : 0).'T from leader, '.$base['senators'].'T from senators, '.$base['knights'].'T from knights and '.($base['total']-($base['leader']!=NULL ? 3 : 0)-$base['senators']-$base['knights']).'T from Concessions.') ;
            foreach ($base['provinces'] as $data) {
                $province = $data['province'];
                $senator = $data['senator'];
                if (is_null($request[$province->id])) {
                    return 'ERROR. Undefined province.';
                }
                ...
                if ($request[$province->id] == 'YES') {
                    // Spoils
                    $message .= $senator->name.' takes provincial spoils for '.$revenue['senator'].'T .'.( ($revenue['senator']<0) ? ' As it\'s negative, it\'s paid by Rome. ' : '') ;
                ...
                if ( !($province->developed)) {
                    $roll = mt_rand(1,6) ;
                    $modifier = ( ($request[$province->id] == 'YES') ? 0 : 1) ;
                    if ( ($roll+$modifier) >= 6 ) {
                        $message.=' A '.$roll.' is rolled'.($modifier==1 ? ' (modified by +1 for no spoils)' : '').', the province is developed. '.$senator->name.' gains 3 INFLUENCE.';
                        $senator->INF+=3;
                    } else {
                        $message.=' A '.$roll.' is rolled'.($modifier==1 ? ' (modified by +1 for no spoils)' : '').', the province is not developed.';
                    }
                }
            ...
            if ($this->whoseTurn() === FALSE ) {
                ...
                array_push ($messages , 'All revenues collected, parties can now redistribute money.') ;
            }
            return $messages ;
        }
    }
    ...
    /**
     * 
     * $fromTI and $toTI are arrays in the form 'type' =>'senator'|'party' , 'id'
     * @param type $user_id
     * @param type $from
     * @param type $to
     * @return string
     */
    public function revenue_DoRedistribute ($user_id , $fromTI , $toTI , $amount) {
        if ( ($this->phase=='Revenue') && ($this->subPhase=='Redistribution') && ($this->party[$user_id]->phase_done==FALSE) ) {
            $from = ( $fromTI['type']=='senator' ? getSenatorWithID($fromTI['id']) : $this->party[$fromTI['id']] );
            $to = ( $toTI['type']=='senator' ? getSenatorWithID($toTI['id']) : $this->party[$toTI['id']] );
            if ($from===FALSE)  { return 'ERROR - Giving from wrong Senator'; }
            if ($to===FALSE)    { return 'ERROR - Giving to wrong Senator'; }
            if (!is_set($from)) { return 'ERROR - Giving from wrong Party'; }
            if (!is_set($to))   { return 'ERROR - Giving to wrong Party'; }
            if ($from->treasury < $fromTI['amount']) { return 'ERROR - Not enough money'; }
            $from->treasury-=$amount ;
            $to->treasury+=$amount ;
            return $from->name.' gives '.$amount.'T to '.$to->name ;
        }
    }
    ...
    public function playStateman( $user_id , $card_id ) {
        ...
        if ($stateman === FALSE ) {
            array_push($messages , 'ERROR - the card is not in [%USER_ID'.$user_id.'%]\'s hand');
            return $messages ;   
        } else {
                if ($stateman->type!='Stateman') {
                    // This is not a stateman, put the card back and report an error
                    $this->party[$user_id]->hand->putOnTop($stateman);
                    array_push($messages , 'ERROR - "'.$stateman->name.'" is not a stateman');
                    return $messages ;
                } else {
                    ...
                    if ($family === FALSE) {
                        ...
                        $this->party[$user_id]->hand->putOnTop($stateman);
                        array_push($messages , 'ERROR - Weird family.');
                        return $messages ;
                    }
                    ...
                    foreach ($this->party[$user_id]->senators->cards as $senator) {
                        if ($senator->senatorID == $family) {
                            ...
                            if ($matchedFamily === FALSE) {
                                ...
                                array_push($messages , '[%USER_ID'.$user_id.'%] plays Stateman '.$stateman->name.' on top of senator '.$matchedFamily->name);
                                return $messages ;
                            }
                        }
                    }
                    ...
                    foreach ($this->forum->cards as $card) {
                        if ( ($card->type == 'Family') && ($card->senatorID == $family) ) {
                            ...
                            $this->party[$user_id]->senators->putOnTop($stateman) ;
                            $stateman->controls->putOnTop($matchedFamily);
                            array_push($messages , '[%USER_ID'.$user_id.'%] plays Stateman '.$stateman->name.' and gets the matching unaligned family from the Forum.');
                            return $messages ;
                        }
                    ...
                    $this->party[$user_id]->senators->putOnTop($stateman) ;
                    array_push($messages , '[%USER_ID'.$user_id.'%] plays Stateman '.$stateman->name);
                    return $messages ;
                }
        }
    }
    ...
    public function playConcession( $user_id , $card_id , $senator_id) {
        ...
        if (!$partyOfTargetSenator) {
            array_push($messages , 'This senator is not in play') ;
            return $messages;
        }
        $senator = $this->party[$user_id]->senators->drawCardWithValue('senatorID', $senator_id);
        if ($senator === FALSE ) {
            array_push($messages , 'ERROR - the senator is not in [%USER_ID'.$user_id.'%]\'s party');
            return $messages ;   
        }
        ...
        if ($concession === FALSE ) {
            ...
            if ($concession->type!='Concession') {
               ...
               array_push($messages , 'ERROR - "'.$concession->name.'" is not a concession');
               return $messages ;
            } elseif($concession->name=='LAND COMMISSIONER' && !$this->landCommissionerPlaybale()) {
               $this->party[$user_id]->senators->putOnTop($senator);
               $this->party[$user_id]->hand->putOnTop($concession);
               array_push($messages , 'ERROR - The Land commissioner can only be played while Land bills are enacted.');
               return $messages ;
            } else {
                $senator->controls->putOnTop($concession);
                $this->party[$user_id]->senators->putOnTop($senator);
                array_push($messages , '[%USER_ID'.$user_id.'%] plays Concession '.$concession->name.' on Senator '.$senator->name);
                return $messages ;
            }
        }
    }
